  ...
  <section>
    ...
    <el-col :span="20":class="this.Global.pageToolBarClass" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.rcName" placeholder="计量单位名称" clearable></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="resultData" highlight-current-row  border v-loading="listLoading" style="width: 100%;" :height="tableHeight"
      ...
      <el-table-column prop="Rcname" label="单位名称"  min-width="200"  header-align="center"  align="center">
      </el-table-column>
      <el-table-column prop="Rcsort" label="排序"  min-width="80"  header-align="center"  align="center">
      </el-table-column>
    ...
    <el-dialog :title="editTitle"  :visible.sync="editFormVisible"  width="50%"  :close-on-click-modal="false">
      ...
      <el-card class="box-card" style="margin:-10px 0px -25px 0px; border:#DCDFE6 1px solid; ">
        <el-form :model="editForm" label-width="160px" :rules="editFormRules" ref="editForm">
          <el-form-item label="单位名称" prop="Rcname" :inline-message="true">
            <el-col :span="12">
              <el-input v-model.trim="editForm.Rcname" placeholder="请输入计量单位名称" maxlength="30" clearable></el-input>
            </el-col>
          </el-form-item>
          <el-form-item label="排序" prop="Rcsort" :inline-message="true" style="margin-top: 10px;">
            <el-col :span="12">
              <el-input-number v-model="editForm.Rcsort" :precision="0" :step="1" :min="1" :max="10000"></el-input-number>
            </el-col>
          </el-form-item>
        </el-form>
      </el-card>
    ...
    methods: {
      ...
      f_Delete: function (index, row) {
        this.$confirm('确定删除吗?', '提示', {
          ...
          this.$httpPost('ResourceConfig/DResourceConfig',pars).then((res)=> {
            if(res.success=='ok') {
      ...
      f_Search: function () {
        ...
        let parms = {
          ...
          size: this.size,
          rcType: 'METERUNIT',
          rcName:this.filters.rcName,
        };
        _this.$httpPost('ResourceConfig/GResourceConfigList', parms).then((res) => {
          if (res.success == 'ok') {
      ...
      f_EditSubmit:function () {
        ...
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            ...
            _this.$httpPost('ResourceConfig/SResourceConfig', pars).then((res) => {
              if (res.success == 'ok') {
                if(!res.addResult){
                  _this.$alert('计量单位名称已存在，请重新输入！', '提示', {
                    confirmButtonText: '确定',
                    type: 'error',
                  });
