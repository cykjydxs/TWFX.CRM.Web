<template>
  <section>
    ...
    <el-table :data="resultData" highlight-current-row  border v-loading="listLoading" style="width: 100%;" :height="tableHeight"
      ...
      <el-table-column prop="Sellcode" label="订单号"  min-width="170"  show-overflow-tooltip  header-align="center" align="center">
        <template slot-scope="scope">
          <span style="color: #409EFF; cursor: pointer;" @click="f_View(scope.$index, scope.row)">{{scope.row.Sellcode}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="Bunkname" label="商铺名称" min-width="160" show-overflow-tooltip   header-align="center"  align="left">
      </el-table-column>
      <el-table-column prop="Pucount" label="数量" min-width="60"  header-align="center"  align="center">
      </el-table-column>
      <el-table-column prop="Pumoney" label="订单金额/元" min-width="90"  header-align="center"  align="center">
        <template slot-scope="scope">
          {{ scope.row.Pumoney | money(2) }}
        </template>
      </el-table-column>
      <el-table-column prop="Isoutstock" label="订单状态" min-width="70"  header-align="center"  align="center">
        <template slot-scope="scope">
      ...
      <el-table-column prop="Sellmanid" label="业务员" min-width="70" show-overflow-tooltip  header-align="center"  align="center">
      </el-table-column>
      <el-table-column prop="Selldate" label="订单日期"  min-width="160"   header-align="center"  align="center">
        <template slot-scope="scope">
        ...
        <template slot-scope="scope">
          <div v-if="pageButtonType===1">
            ...
            <el-tooltip class="item" effect="dark" content="打印" placement="top">
              <el-button size="mini" type="primary" icon="el-icon-printer" circle  @click="f_Print(scope.$index, scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="出库" placement="top" v-show="scope.row.Isoutstock!='Y'">
          ...
          <div v-else-if="pageButtonType===2">
            ...
            <el-button size="mini" @click="f_Print(scope.$index, scope.row)">打印</el-button>
            <el-button size="mini" @click="f_InStock(scope.$index, scope.row)" v-show="scope.row.Isoutstock!='Y'">出库</el-button>
          ...
          <div v-else>
            ...
            <el-button type="text"  @click="f_Print(scope.$index, scope.row)">打印</el-button>
            <el-button type="text"  @click="f_InStock(scope.$index, scope.row)" v-show="scope.row.Isoutstock!='Y'">出库</el-button>
      ...
      <el-tabs v-model="activeTabName" type="border-card" style="margin:-20px 0px -20px 0px; border:#DCDFE6 1px solid; height: 450px;">
        <el-tab-pane label="基本信息" name="tabedit">
          ...
          <el-form :model="editForm" label-width="180px" :rules="editFormRules" ref="editForm" style="margin:-20px -20px -20px -20px; border:#DCDFE6 1px solid;">
            ...
            <el-form-item label="商铺名称" prop="Bunkname" :inline-message="true" style="margin-top: 10px;">
              <el-col :span="12">
                <el-input v-model.trim="editForm.Bunkname" placeholder="请选择商铺" maxlength="30" :disabled="true">
                  <el-button slot="append" icon="el-icon-search" @click.native="f_DataSelectSupplier"></el-button>
                </el-input>
              </el-col>
            </el-form-item>
            <el-form-item label="订单日期" prop="Selldate" :inline-message="true">
              <el-col :span="12">
                <el-date-picker
            ...
            <el-form-item label="订单说明" prop="Selldesc" :inline-message="true" style="margin-top: 10px;">
              <el-col :span="16">
                <el-input type="textarea" v-model="editForm.Selldesc" placeholder="请输入采购说明" maxlength="100" :rows="2" clearable></el-input>
              </el-col>
            </el-form-item>
            <el-form-item label="订单金额" prop="Pumoney" :inline-message="true" style="margin-top: 10px;">
              <el-col :span="8">
            ...
            <el-form-item label="订单数量" prop="Pucount" :inline-message="true" style="margin-top: 10px;">
              <el-col :span="12">
        ...
        <el-tab-pane label="订单产品" name="tablist">
          <span slot="label"><i class="el-icon-circle-plus"></i> 订单产品</span>
          ...
          <el-table :data="puData" highlight-current-row  border v-loading="listLoading" style="width: 100%; "  :height="350"
            ...
            <el-table-column prop="Puprice" label="供货价/元" min-width="140"  header-align="center"  align="center">
              <template slot-scope="scope">
    ...
    <el-dialog :title="editTitle"  :visible.sync="viewFormVisible"  width="65%"  :close-on-click-modal="false">
      <div slot="title" style="font-size: 14px; font-weight: bold; color: #606266; " >
        <i class="fa fa-th-large" aria-hidden="true" style="margin-right: 10px;"></i>订单查看
      </div>
      <el-tabs v-model="activeTabName" type="border-card" style="margin:-20px 0px -20px 0px; border:#DCDFE6 1px solid; height: 450px;">
        <el-tab-pane label="基本信息" name="tabedit">
          <span slot="label"><i class="el-icon-tickets"></i> 基本信息</span>
          <el-form :model="editForm" label-width="180px" :rules="editFormRules" ref="editForm" style="margin:-20px -20px -20px -20px; border:#DCDFE6 1px solid;">
            ...
            <el-form-item label="商铺名称" prop="Bunkname" :inline-message="true" style="margin-top: 10px;">
              <el-col :span="12">
                <el-input v-model.trim="editForm.Bunkname" placeholder="请选择商铺" maxlength="30" :disabled="true"></el-input>
              </el-col>
            </el-form-item>
            <el-form-item label="订单日期" prop="Selldate" :inline-message="true">
              <el-col :span="12">
        ...
        <el-tab-pane label="订单产品" name="tablist">
          ...
          <el-table :data="puData" highlight-current-row  border v-loading="listLoading" style="width: 100%; "  :height="350" >
            ...
            <el-table-column prop="Puprice" label="供货价/元" min-width="80"  header-align="center"  align="center">
              <template slot-scope="scope">
    ...
    <el-dialog  :visible.sync="dataSelectsupplierVisible"  width="60%"  :close-on-click-modal="false">
      ...
      <el-table :data="supplierList" highlight-current-row  border v-loading="dataSelectProductLoading" style="width: 100%;" height="300"
                @row-click="f_DataSelectSupplierClick" @row-dblclick="f_DataSelectSupplierDbClick" :header-cell-style="this.Global.headerCellStyle" >
        <el-table-column type="index" width="60"  label="序号"  header-align="center" align="center">
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      f_Edit: function (index, row) {
        ...
        this.f_EditGetDetail(row.Sellid);
      },
      //编辑
      f_EditGetDetail:function(pkid){
        ...
        let _this=this;
        this.$httpPost('Sell/GSellEdit',pars).then((res)=> {
          if(res.success=='ok') {
      ...
      f_Delete: function (index, row) {
        ...
        this.$confirm('确定删除吗?', '提示', {
          ...
          _this.$httpPost('Sell/DSell',pars).then((res)=> {
            _this.listLoading = false;
            if(res.success=='ok') {
              _this.listLoading = false;
              _this.$message({
      ...
      f_InStock:function(index,row){
        let _this=this;
        this.$confirm('出库会生成库存流水，确定出库吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          ...
          _this.$httpPost('Sell/SOutStock',pars).then((res)=> {
            _this.listLoading = false;
            if(res.success=='ok') {
              _this.listLoading = false;
              _this.$message({
        ...
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            if(_this.puData==null||_this.puData.length<=0) {
              ...
              _this.$httpPost('Sell/SSell', pars).then((res) => {
                if (res.success == 'ok') {
      ...
      f_Print:function(index,row){
          ...
          this.$confirm('确定打印吗?', '提示', {
            ...
            _this.$httpPostUp('Sell/GetAccessExcel', pars, config).then((res) => {
              _this.listLoading = false;
              if (!res) {
                  _this.$message({
                    showClose: true,
                ...
                link.href = url;
                link.setAttribute('download', '送货单.xls');
      ...
      f_ConfigLoad:function(){
        let _this=this;
        this.$httpPost('Sell/GEditConfig',{}).then((res)=> {
          if(res.success=='ok') {
      ...
      f_Search: function () {
        ...
        //url参数
        let sellId = this.Global.isNullOrEmpty(this.$route.query.sellId) ? 0 : this.$route.query.sellId;
        if (sellId > 0)
          this.filters.sellId = sellId;
        ...
        let parms = {
          ...
          sellId:this.filters.sellId,
        };
        _this.$httpPost('Sell/GSellList', parms).then((res) => {
          if (res.success == 'ok') {
